...
- **Repo shape**: single Git repository (monorepo) with three top-level entries: `host/`, `plugins/`, and repo-root meta-files (`README.md`, `CLAUDE.md`, `Makefile`, `.gitignore`, `issues/`). Nothing app-specific lives loose at repo root.
  ...
  │   └── package.json
  └── plugins/
      ├── shared-ui/            # branding + generic UI components
      │   ├── src/SharedUiServiceProvider.php
...
- **Tool contract package**: `plugins/tool-registry`, a peer of `shared-ui` under `plugins/`, wired the same path-repository way. It exposes:
  ...
  - `ToolRegistry` — a plain in-memory registry with `register(ToolContract $tool): void` and `all(): Collection` (or array), bound as a **singleton** in the container by `tool-registry`'s own `ToolRegistryServiceProvider`.
- **Registration flow**: each plugin ships one class implementing `ToolContract` (e.g. `<ToolName>Tool`) describing itself. In its own `ServiceProvider::boot()`, the plugin resolves `ToolRegistry` from the container and calls `register(new <ToolName>Tool())`. There is no central list maintained in `host/` — a plugin declares itself, and the registry simply accumulates whatever booted. Because Laravel runs every discovered provider's `register()` phase before any provider's `boot()` phase, binding the `ToolRegistry` singleton in `tool-registry`'s `register()` guarantees it's available by the time any tool plugin's `boot()` tries to register into it, regardless of provider load order.
- **Host consumption / tool listing**: `host/` depends on `tool-registry` (to read) and `shared-ui` (to render), but not on any individual tool plugin. A route/controller (e.g. the home page) resolves `ToolRegistry::all()` and passes it to a Blade view that renders a **grid of cards**, one per registered tool, each showing `icon()`, `name()`, and `description()` and wrapping the card in a link to `url()`. The grid itself is a `shared-ui` component so its look is consistent with the rest of the host's chrome. `host/` never imports or references a specific plugin's classes — it only ever depends on the `ToolContract` type.
- **View/asset namespacing**: each plugin registers its Blade views under its own view namespace (e.g. `<plugin>::`) to avoid collisions; `shared-ui`'s components are registered under one stable namespace (e.g. `brand::` or an anonymous component namespace) so both host and plugin views can reference them identically.
...
- Icon format/storage (emoji, SVG string, asset path, icon-font class) — `ToolContract::icon()` returns a `string`; what that string represents is left to the build phase and should be documented once decided, likely alongside the `shared-ui` card component's expectations.
...
- The structural decisions confirmed with the user before writing this spec: (1) plugins are wired via Composer **path repositories** as real standalone packages, not a plain extra-PSR-4-autoload-entry approach; (2) shared branding/UI lives in its **own dependency package** (`plugins/shared-ui`), not published-by-main-app-and-consumed; (3) the main app is isolated into its own root-level `host/` folder rather than living at repo root, so repo root is reserved for `README.md`, `CLAUDE.md`, `Makefile`, and `plugins/`; (4) tool discovery/listing goes through a **shared contract + registry package** (`plugins/tool-registry`) rather than a hardcoded list in `host/` — each plugin self-describes via `ToolContract` and self-registers into `ToolRegistry` at boot.
