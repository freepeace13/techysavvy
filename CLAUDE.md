...
- New tools implement `Techysavvy\ToolRegistry\ToolContract` and register themselves via `ToolRegistry::register()` inside their own `ServiceProvider::boot()` — never add a tool to a list inside `host/`.
- Branding/UI: use `plugins/shared-ui`'s Blade components (`<x-brand::...>`) instead of duplicating markup/styles in a plugin or in `host/`.
