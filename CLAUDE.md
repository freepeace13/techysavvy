...
- Plugin CSS/JS is registered with core, not wired by hand: build it in the plugin, declare it with `AssetRegistry::register()` + `AssetBundle` in the plugin's `ServiceProvider::boot()`, and request it from a view with `@pluginAssets('<name>')`. Never hand-write `<script>`/`<link>` tags for plugin bundles, add them to `host/`'s Vite build, or use `vendor:publish` for them.
- Branding/UI: use `plugins/ui`'s Blade components (`<x-ui::...>`) instead of duplicating markup/styles in a plugin or in `host/`.
